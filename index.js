...
function isDeclaration(statement) {
  ...
  return (
    statement[0] &&
    (statement[0].type === "DeclarativeExpression" ||
      statement[0].type === "AssignmentExpression")
  );
}
...
function generateInterface(src, options = {}) {
  ...
  const pragma = ast.body.find(
    (statement) => statement.type === "PragmaStatement"
  );
  const pragmaSrc = pragma ? src.slice(pragma.start, pragma.end) + "\n\n" : "";

  // get contract name
  const contract = ast.body.find(
    (statement) => statement.type === "ContractStatement"
  );
  const supers = contract.is
    .map((supercontract) => supercontract.name)
    .reduce(toLookupObject, {});

  const importStubs = ast.body
  ...
  const enumNames = contract.body
    ? contract.body.filter(isEnum).map((en) => en.name)
    : [];
  ...
  const functions = contract.body
    ? contract.body
        ...
        .map((f) => {
          f.notModifiers = f.modifiers
            ? f.modifiers.filter((mod) => mod.name in notModifiers)
            : [];
          return f;
        })
    : [];
  ...
  const getters = contract.body
    ? contract.body
        .filter(isDeclaration)
        .map((declaration) => declaration[0].left || declaration[0])
        .filter(isPublicDeclaration)
    : [];
  const getterStubs = getters.map(
    (getter) =>
      `  function ${
        getter.name
      }() public constant returns(${getGetterReturnType(getter)});`
  );
